...
- **Anthropic model selection:** the pipeline uses `claude-opus-4-7` or `claude-sonnet-4-6` depending on phase — check `brief/claude/*.py`. Don't downgrade to older models; don't bump to newer ones without confirming the prompt contract still holds.
