...
EconDelta re-upserts the standing BB policy corridor to `metric_history` every day, so `policy_rate_repo` / `policy_rate_sdf` / `policy_rate_slf` rows always carry `as_of` = the run date, not the MPC decision date. Read them for the VALUE via `history.get_latest`, keep `cadence="event"`, and never present that `as_of` as "the rate changed on this date." `brief/builders/bb.py` reads these live (**repo/sdf/slf = 9.50/7.50/11.00 as of the 2026-07-30 MPC cut**; the retired hardcoded 8.5 SDF is gone). Live reserves id is `gross_reserves_usd_bn`; the legacy `bb_gross_reserves` id has had **no writer since 2026-03-01** — do not read it (extends landmine #6's live-vs-legacy map). **It is NOT "fresh daily" any more (corrected 2026-09-02):** since econdelta #97 (1 Aug 2026) EconDelta stamps the row with BB's month-END date (`as_of` 2026-06-30, 2026-07-31, …) and the new month's figure only lands ~11 days after month-end, yet `bb.py` and `fx.py` both declared it `cadence="weekly"` (fresh ≤7d, stale >10d) until PR #208 (4 Sep 2026) re-declared it `monthly` in lockstep — `tests/builders/test_bb.py::test_reserves_as_of_matches_fx_builder` pins the two builders' reserves cadence EQUAL; never let them split. A weekly clock on a month-end-stamped feed was stale on arrival and stayed stale until the next month landed: §02 (`bb`) has read "stale" in every issue since #98 (119/119 — though only from ~10 Aug, when the first month-end row was written, is the reserves clock the driver; before that the corridor rows simply did not exist yet and `bb` fell back to `stale=True` constants), and FX & External (`fx`) since #205, when #167 made its freshness worst-of-all-metrics. Under `monthly` (fresh ≤35d, warning ≤45d) BB's ~12-day publication lag means the LAST ~7 DAYS of every cycle read `warning` (AGING) BY DESIGN — a test pins day 41 as `warning`; that is the chosen trade-off, not a regression, and a reserves-specific threshold is the alternative if it proves noisy. `fx` still reads stale after #208 while its June exports/trade-gap tiles sit past 45 days (parked EPB appender, landmine 27). Cadence is not badge-only: it also feeds `pipeline_v6._days_since_refresh` → the daily lens scorer's freshness term (a `stale` section scores 0.0 there and can never win the lens), and `vintage.period_label` ("Jul 2026" for monthly vs "31 Jul 2026" for weekly). When a corridor read fails (history unreachable / row missing), `bb.py` falls back to a module last-known constant marked `stale=True` — never blank, never a fallback mislabelled as live.
...
## 39. `brief.service` execs whatever `CLAUDE_BINARY` in `/etc/brief.env` points at — an npm-global Claude Code that Copotron AUTO-UPDATES in place. A `PermissionError` on that path is a half-installed update, not a permissions problem, and a full disk is the usual cause

12–16 Sep 2026: issues 226–229 never built. Each 08:00 BDT fire died in ~25 s, straight after `v6: issue_no=…, 10 sections raw`, with `PermissionError: [Errno 13] Permission denied: '/home/adnan/.npm-global/bin/claude'` raised from the `subprocess.run` in `run_max` (`brief/claude/max_client.py`). The OnFailure Discord alert fired all four mornings (HTTP 204) and went unread. Nothing in this repo changed (HEAD cc5e946 throughout). What actually happened, on the box:

1. **How the binary is wired.** `/etc/brief.env` sets `CLAUDE_BINARY=/home/adnan/.npm-global/bin/claude` → symlink → `…/@anthropic-ai/claude-code/bin/claude.exe`. Since Claude Code 2.1.2xx the npm package ships `bin/claude.exe` as a 500-byte, mode-644 SHELL STUB ("claude native binary not installed"); the `postinstall` (`install.cjs`) hard-links the ~220 MB native binary from the optional dep `@anthropic-ai/claude-code-linux-x64` over it. If postinstall never completes, the stub stays, is not executable, `exec` fails with EACCES, Python raises `PermissionError`, and `run_max` does not catch it (it only maps `FileNotFoundError` to `MaxCallError`) — exit 1, no retry. `/usr/bin/claude` on the box is a separate root-owned 2.1.104 from April; it is NOT a fallback the service can use.
2. **Who reinstalls it.** Copotron runs from the same npm-global install with `installMethod: "global"`, so Claude Code's auto-updater runs `npm install --global @anthropic-ai/claude-code@<latest>` in the background seconds after every bot launch (confirmed 16 Sep 16:10 UTC: launch at :10:01, npm log at :10:13, "Update installed · Restart to apply" in the pane). On 12 Sep 18:00 UTC one such install succeeded; a second at 18:30 UTC hit ENOSPC halfway through extracting the 220 MB binary (117 MB on disk) because the root disk reached 100% at 18:32 UTC (rsyslog "No space left on device"). npm left the previous tree behind as `…/@anthropic-ai/.claude-code-<random>/` — a complete, working install; renaming it back into place is the rollback (no download, no disk needed).
3. **Diagnose in this order.** `logs/brief-systemd.log` traceback → `ls -la "$(readlink -f ~/.npm-global/bin/claude)"` (expect ~220 MB `-rwxr-xr-x`; a 500-byte `-rw-rw-r--` is the stub) → `df -h /` → `ls -la ~/.npm-global/lib/node_modules/@anthropic-ai/` for a retired `.claude-code-*` tree → `sudo journalctl --since <date> | grep -i 'no space'`. If the stub is there and a retired tree exists: `mv claude-code <aside> && mv .claude-code-XXXX claude-code`, then `claude --version`. Otherwise free disk first, then `npm install -g @anthropic-ai/claude-code@<ver>`. A manual `sudo systemctl start brief.service` afterwards publishes the missed day (issue 226 went out at 22:33 BDT this way).
4. **What filled the disk was not this project.** `/opt/chess-prodigy/releases` kept every deploy (20 × ~465 MB = 9.3 GB in one week) plus 1.9 GB of `/tmp/chess-*-stage.*` leftovers; npm's cache was 3.5 GB; 7 disabled snap revisions ~1.5 GB. The box was at 65% on 5 Sep and 100% on 12 Sep. Every claude caller on the box — this service, Copotron, `manifest-brief` — shares that disk and that binary, so one tenant's growth takes all of them down.

Prevention is tracked in `AGENT_LEARNINGS.md` (2026-09-16): `DISABLE_AUTOUPDATER=1` in the box's `~/.claude/settings.json` `env` block, a disk-free floor plus `test -x "$CLAUDE_BINARY"` in `deploy/brief_guard.sh` so the alert names the cause, and a watchdog that alerts after repeated relaunch failures. The guard checks have shipped (`deploy/brief_guard.sh`, `tests/test_brief_guard.py`); the auto-update and watchdog items are tracked separately. A same-day review caught that the guard's refusal text was landing only in `logs/brief-systemd.log` (from `brief.service`'s `StandardOutput=`/`StandardError=append:`), never in the journal `brief_alert.sh`'s Discord alert reads — so "the alert names the cause" was false until the guard was also wired to `logger` (journald), which it now is.
