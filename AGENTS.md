...
## 34. The post-editor number/period validator (`brief/validators/prose_numbers.py`) checks prose against the BUILDER's raw values, never the editor's own formatted string — and only two of its three BLOCK checks are unconditional

P2 fact-checker (2026-08-22 audit #204, round-2 follow-up). A round-2 engineering review rejected the first cut of this idea outright: "prose-vs-payload would have passed every wrong number because the payload itself carries them — validate against the BUILDER/source values." Every check in this module therefore reads `sections_raw` (pre-editor, deterministic) as ground truth, never `MetricV6.value`.

Two severities, wired into `run_publish` right before publish (`pipeline_v6._run_prose_number_gate`), after every other deterministic stamp has had its say on `sub` text:

- **BLOCK** (hard-fails via `ProseNumberGateError`, a `V6PublishError` — exit code 4, same as the denylist check): every currency/percent/bp figure in a metric's `sub` must trace to that section's raw values (its own, a sibling's, or a pairwise `|a−b|` derived delta, e.g. "19bp under the 9.50% policy") within tolerance = half a unit in the LAST PRINTED DIGIT of the prose token (an integer "733" tolerates ±0.5, "9.50" tolerates ±0.005); every month-name token in a `sub` must equal that metric's own data period or a same-section sibling's, sourced from `as_of` — **except event-cadence metrics** (the BB policy corridor), whose `as_of` is a daily restamp date, not a decision date (landmine 24) — a sub naming the real decision month ("held since the 30 Jul cut") is legitimate and exempted from this check entirely; and a sourceless `"across/for/in N reads/prints/sessions/days"` count-claim anywhere in the brief's prose is blocked outright (the pipeline never supplies a count field, so this is an effective ban).
- **WARN** (logged + Discord-alerted via `_alert`, never blocks): every currency/percent figure in `todays_call`/`tldr`/`verdict`/`analysis` with no builder match ANYWHERE in the issue (union across sections). `BRIEF_PROSE_VALIDATOR_STRICT=1` upgrades this to the same hard-fail — a documented future flip, not the default; flip it only after the WARN log volume is known.

Numbers with no currency symbol and no recognized unit suffix (`%`/`bp`/`bn`/`mn`/`cr`/`tn`) are skipped entirely by BOTH severities — a bare digit is far more often a year, an ordinal, or an unrelated count than a value assertion, and precision matters more than recall here. Negative figures use Master.md's minus GLYPH (−, U+2212), captured separately from the ASCII hyphen and the em-dash (—, U+2014) sentence separator so none of the three collide.

`period` is a NEW field on every raw metric in `sections_raw[].metrics[]` (`brief/vintage.py`'s `period_label`, exported publicly for this), always present — unlike `vintage`, which only exists past a metric's fresh threshold. `series_summary` is a NEW per-section field (n/first_ts/first_value/last_ts/last_value/min/max, built by `pipeline_v6._fetch_series_summaries`) fixing the prior lie that the editor's raw input carried a chart's actual `series`/`notes` — it never did; those are stamped onto the FINAL brief post-editor (`_stamp_chart_series`, unmoved, unchanged) for payload-size reasons. `chart_read` must derive only from `series_summary`.
