...
**Verification note (honesty, not hedge): this rewrite is sourced from the repo's own 2026-08-08 incident record (AGENT_LEARNINGS.md), NOT a fresh live query.** The review round that produced this rewrite asked for a live anon-key `curl` against production as primary evidence; the session doing the rewrite had no `NEXT_PUBLIC_SUPABASE_ANON_KEY` available (no `.env.local`, not in the shell environment, and `app/page.tsx`'s use of it is server-side only, so it doesn't leak into the deployed site's client bundle). Adnan: run the recipe below (or hand over the anon key) to independently confirm before trusting this landmine at face value:

```bash
set -a && . ./.env.local && set +a
for id in remittance_usd_mn_monthly exports_usd_mn_monthly imports_usd_mn_monthly; do
  curl -s "$NEXT_PUBLIC_SUPABASE_URL/rest/v1/metric_history_monthly?metric_id=eq.$id&select=as_of,value&order=as_of.desc&limit=2" \
    -H "apikey: $NEXT_PUBLIC_SUPABASE_ANON_KEY" -H "Authorization: Bearer $NEXT_PUBLIC_SUPABASE_ANON_KEY"
done
```
Expected (per the 2026-08-22 audit's own figures, themselves independently verified twice before this PR): `remittance_usd_mn_monthly` latest = 2858.68 @ 2026-07 (2816.96 @ 2026-06 second row); `exports_usd_mn_monthly` latest = 4202.69 @ 2026-06; `imports_usd_mn_monthly` latest = 5826.2 @ 2026-03.
