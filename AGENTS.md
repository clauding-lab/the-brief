...
Prevention is tracked in `AGENT_LEARNINGS.md` (2026-09-16): `DISABLE_AUTOUPDATER=1` in the box's `~/.claude/settings.json` `env` block, a disk-free floor plus `test -x "$CLAUDE_BINARY"` in `deploy/brief_guard.sh` so the alert names the cause, and a watchdog that alerts after repeated relaunch failures. The guard checks have shipped (`deploy/brief_guard.sh`, `tests/test_brief_guard.py`); the auto-update and watchdog items are tracked separately.
