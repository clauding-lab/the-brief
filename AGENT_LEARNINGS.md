...
## 2026-09-16 — v2.5.0 | Four issues missed (13–16 Sep): a Claude Code self-update ran out of disk mid-install and left a non-executable stub where `claude` should be

**Trigger:** `/resume-session` orientation on 16 Sep: the latest `briefs` row was issue 225 (12 Sep). Four OnFailure Discord alerts (13–16 Sep, 08:00 BDT, HTTP 204 each) had gone unread. Copotron had been dead since its Saturday rotation (13 Sep 04:00 BDT) with 1,079 silent "restart FAILED" watchdog ticks.

**What went wrong:** The Hetzner root disk hit 100% at 00:32 BDT 13 Sep (a new tenant, `chess-prodigy`, hoarded 9.3 GB of releases in a week; npm cache 3.5 GB; old snap revisions). At that moment Claude Code's auto-updater (npm-global install, triggered by a Copotron launch) was reinstalling `@anthropic-ai/claude-code`; extraction of the 220 MB native binary died at 117 MB and postinstall never replaced the package's 500-byte mode-644 `bin/claude.exe` stub. `brief.service` execs that path (`CLAUDE_BINARY` in `/etc/brief.env`) → `PermissionError` in `run_max` → exit 1 in 25 s, not retried. Upstream (EconDelta aggregate) was healthy every night; the code on the box was unchanged. Mechanics in AGENTS.md landmine #39.

**Lesson:** An unattended box that self-updates its own tools needs the update turned off, a disk-free floor, and a pre-flight that proves the binary is executable — and every relaunch loop must alert after N failures, not just log them.

**Prevention:** (1) `DISABLE_AUTOUPDATER=1` in the box's `~/.claude/settings.json` `env` block (docs: code.claude.com/docs/en/setup, "Disable auto-updates"); updating becomes a deliberate `claude update`. (2) `deploy/brief_guard.sh`: fail closed when `df` free < 2 GB or `! test -x "$CLAUDE_BINARY"`, with a message that names the cause, so the OnFailure alert says WHY. (3) `~/bin/copotron-watchdog.sh`: Discord alert after 3 consecutive "restart FAILED" ticks (throttled like the auth-wall alert). (4) chess-prodigy deploy: keep the last 3 releases, clean `/tmp/*-stage.*`. (5) A box-level disk alert at 85%. (6) Someone reads the alert channel: four alerts were delivered and ignored — alerting worked, reading did not. (7) A refusal message only reaches an alert if it travels the same channel the alert reads: `brief.service`'s `StandardOutput=`/`StandardError=append:` writes this unit's stdout/stderr straight to `logs/brief-systemd.log`, not the journal `deploy/brief_alert.sh` builds its Discord payload from, so `brief_guard.sh`'s `_refuse` now also writes through `logger` (journald) — a lesson worth generalizing to any guard/refusal script on a unit with file-redirected `StandardOutput`/`StandardError`.

**Hotfix (16 Sep, 22:00–22:35 BDT, owner-approved batch):** freed 3.9 GB without deleting anything (npm cache clean, journal vacuum to 200 MB, 7 disabled snap revisions removed); staged 9.8 GB of approved deletes in `/home/adnan/.trash-20260916/` (4 `/tmp` stage dirs, 17 old chess-prodigy releases, the broken claude tree) for the owner to remove; renamed npm's retired `.claude-code-bDq1Jpch` tree back into place (`claude --version` → 2.1.269); the watchdog relaunched Copotron at 22:10 BDT and the auto-updater immediately installed 2.1.273 cleanly now that disk was available; a manual `systemctl start brief.service` published issue 226 at 22:33 BDT (10 sections, `sent=10/10`, live at `/issue/226`). No code changed.

**Cross-references:** AGENTS.md landmine #39; auto-memory `project_claude_binary_outage_2026_09_12.md`; global rulebook 2026-09-16 "Hetzner / Copotron / The Brief"; the 2026-09-04 global entry (watchdog must read the screen) is the sibling gap.
...
**What went wrong:** Bangladesh Bank rebuilt its `gsom.bb.org.bd` treasury pages around 1 Jul. EconDelta's `parse_all` masked the resulting "File not found" with a 60-day `stale_fallback`, which expired on 31 Aug; the four treasury ids then dropped out of the aggregate and `_quarantine_flagged`'s deterministic rule ("any flagged id absent from `data`, or more than `MAX_QUARANTINE_FIELDS` flagged at once → hard reject") kept yesterday's `latest.json`. Nothing was written to `metric_history` after 30 Aug, so EVERY aggregate-owned id — `dsex`, `dommr`/`bofr`, `usd_bdt_*`, call money, the policy corridor — froze together, while `dse_close_*` (a separate scraper) kept flowing. The Brief's publish gate does not block on a stuck upstream, so issues 213–217 published on time with honest stale badges and nobody was alerted on this side. The log line blamed the Opus reviewer ("opus review REJECTED (hard)"); the reviewer was the messenger. A second, independent cause rode along: the fiscal-year-to-date export/remittance series reset in July and read as a ~90% collapse.

The fix took four EconDelta PRs, not two. #133 (URL repoint) + #134 (FY-reset guard) merged 2 Sep and self-deployed at 01:00 BDT 3 Sep — but the 3 Sep 02:55 run only "passed" because the Opus review itself was skipped (`claude_exit_1`) and the upsert went through unreviewed, and the 03:16 retry hard-rejected again on `treasury_bill_outstanding`. #135 (guard hardening) + #136 (fetch the gsom page for an explicit date; alarm on stale holdovers) merged 3 Sep; the first clean reviewed fire was 4 Sep 02:56 BDT (146 rows, three OK runs that day). Only `dse` recovered on issue 217 (4 Sep): `bb`/`fx` stay stale on the reserves-cadence mis-declaration (landmine 24, fixed by #208), `tbond`/`remit` on their own cadence/appender lags (24/27), and `banking`/`fiscal`/`macro` on archive metrics by design — 6/10 sections read stale on a HEALTHY morning. That baseline is why the alarm has to be "newly stale vs yesterday", never "several sections stale".
...
**What went wrong:** `_OFFICIAL_CPI_SOURCES` was written as `frozenset({"bb_inflation_page"})` — a single-source allowlist. That correctly excludes `derived_implied_weight_bb_inflation` (the one point it was actually designed to catch — July food's arithmetic-derived 7.16) but it also excludes `macro_observer_seed`, the Phase-1 seeded backfill that carries **every CPI-trio row before 2026-04-01** (513 of 525 rows in production, 2012-01 through 2026-03). Nobody checked the source composition of the OLDER rows before shipping the allowlist — every fixture and every piece of evidence in the original investigation only sampled `as_of >= 2026-05-01`. `fetch_macro_cpi_series(months=24)` fetches the newest 72 rows (24 × 3 series) globally by `as_of desc`, so the allowlist silently dropped ~20 of every series' 24 fetched months, collapsing the CPI Trend chart to 3-4 points on the next publish — while the chart's own hardcoded subtitle ("24-month · …") and the CHANGELOG's claim that "only the chart's plotted endpoint moved" both stayed unchanged and became false. The one test spanning that window, `test_macro_section_series_populated_with_cpi_24_months`, used `source="bb_inflation_page"` for its 2024/2025 fixture rows — the real producer stamps `macro_observer_seed` there — so it measured quantity/ordering correctly but never exercised the real allowlist gap; it passed green throughout.

**Lesson:** an "official source" allowlist is a deny-by-default filter — it must be checked against the FULL date range it will actually see in production, not just the recent window the investigation happened to sample. A fixture that relabels a real producer's `source` string to make an assertion pass is drift, not coverage; when a test's fixture source doesn't match what the real client returns for that date range, the test is validating the wrong claim.

**Prevention:** `_OFFICIAL_CPI_SOURCES` now also allows `macro_observer_seed` (real historical BB CPI data from the 2026-08-08 frozen-charts backfill, not a fabrication or derivation); `derived_implied_weight_bb_inflation` and the owner-pending denylist entry are unaffected and still excluded. New regression test `test_fetch_macro_cpi_series_keeps_the_seeded_history_not_just_the_live_tail` (`tests/test_pipeline_v6_chart_series_propagation.py`) seeds a fixture with the real producer's source mix (20 `macro_observer_seed` months + 4 `bb_inflation_page` months) and asserts 24/23/23 points survive, not 4/3/3 — confirmed RED (4 == 24 failure) against the pre-repair allowlist and GREEN after. `test_macro_section_series_populated_with_cpi_24_months`'s fixture now stamps `macro_observer_seed` for its 2024/2025 rows to match the real producer. Verified against live production (2026-08-24, same anon read the reviewers used): post-fix, 24/24 months survive for `cpi_12m_avg_monthly`, 23/24 for food and non-food (only each series' own unofficial July point drops) — so the CHANGELOG's "only the chart's plotted endpoint moved" claim and the chart's "24-month" subtitle are now both materially true, not just intended.

**Hotfix:** `brief/chart_series_fetcher.py` (`_OFFICIAL_CPI_SOURCES`), `tests/test_pipeline_v6_chart_series_propagation.py` (new regression test), `tests/test_pipeline_v6_macro_enrichment.py` (fixture source fix), `CHANGELOG.md` (Unreleased entry + production-verified figures).

**Cross-references:** the 2026-08-24 issue-206 entry immediately below (the fix this repairs), AGENTS.md landmine 27 (`metric_history_monthly` archive semantics), AGENT_LEARNINGS 2026-08-08 (`macro_observer_seed`'s original backfill and provenance).
