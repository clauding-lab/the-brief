...
### Changed
- **The govt yield ladder (FIG.03) now plots three month-ends, and no longer waits for a month to finish** (#TBD): on 31 August the chart still read "10Y 10.24% · JUL 2026". Two causes, both real. The chart drew the last **two** month-ends — enough to show that the curve moved, not enough to show whether that was a trend or a one-month jump — so it now draws **three**, the newest solid in accent, the two priors progressively fainter and more finely dashed. And EconDelta's ladder writer only ever published the most recently *completed* month, so August's rung could not exist before 1 September; it now also publishes the current, still-open month as soon as that month has an auction of its own, refreshing it in place as later auctions land (econdelta PR, same day). The consequence in the missing issue: BD govt yields fell ~80–130bp across the whole curve during August (10Y 10.24 → 9.23, 20Y 10.40 → 9.13, part-inverting against 2Y at 9.39) while the page printed July.

### Added
- **The yield ladder names the auction it was built from** (#TBD): a footnote under the chart — "Curve built from auctions through 27 Aug 2026". The ladder's month rungs are stamped to the FIRST of the month, which is a bucket label, not an observation date; now that the newest line can be a *partial* month, "Aug 2026" alone doesn't tell a reader whether it means the 5th or the 27th. The date is the real per-tenor auction date EconDelta already records in `metric_history_monthly.source_as_of`, taken as the newest across the eight tenors of the newest month. Absent on older issues whose rows predate that column — the footnote then simply doesn't render.

### Fixed
- **The Gross Reserves tile is no longer stale on arrival, and §02's freshness signal carries information again** (#208): `gross_reserves_usd_bn` has been month-END stamped since EconDelta began writing it that way (first month-end row 10 Aug 2026, econdelta #97), and Bangladesh Bank's figure lands ~12 days after month-end — but both `bb.py` and `fx.py` declared it `cadence="weekly"` (fresh ≤7 days, stale >10). On that clock the print was stale from day 11 of every cycle, so §02 Policy & Rates has read "stale" in every issue since (and FX & External since #205, when #167 made its signal worst-of-all-metrics). Both builders now declare `monthly` (fresh ≤35 days, warning ≤45, stale beyond), in lockstep — a test pins that the two can never split. With the ~12-day lag the last ~7 days of each cycle read `warning` (the AGING chip) by design; a test pins day 41 as warning so the choice is deliberate. Honest scope: the value and its date do not change; FX & External still reads stale today because its June exports final is 66 days old (parked EPB appender, landmine 27); and freshness is not badge-only — the daily lens scorer's freshness term for §02 moves from 0.0 (a stale section could never win the lens angle) to 1.0 on most days, and the editor's period label for the tile reads "Jul 2026" instead of "31 Jul 2026". The degraded path that dates reserves from the snapshot's month label now normalizes it to month-end, so it cannot re-create the defect when the history read is empty.
...
### Fixed
- **One reserves figure, two dates** (#185): §02 stamped the $36.42bn gross-reserves print "1 Jul" while §04 said "31 Jul" for three straight editions — `bb.py` dated reserves from EconDelta's month *label* (parsed to the 1st) while `fx.py` read the month-END `metric_history` row. Both sections now date reserves from the same history row; a test pins that the two builders can never disagree again.
- **Real Policy Rate overstated by 50bp** (#186, owner-approved): the published 1.68% subtracted July inflation (8.32%) from a 10.00% repo rate that had already been cut to 9.50% on 30 July — the data feed restamps the standing rate daily and didn't record the cut until 3 August, so "at or before 31 July" returned the pre-cut row. A restamp-lag guard now takes the rate in force (honest figure: **1.18%**), bounded at 4 months' vintage gap, with the June-style no-decision pairing unchanged (0.84%). Guard tests pin the ordering logic, not the calendar.
- **The provenance stamp skipped its own debut** (#188): idempotence keyed on the marker phrase "p2p CPI", and issue 210's editor naturally wrote "repo above p2p CPI" — marker present, stamp silently skipped, the corrected 1.18 published with no arithmetic shown. Both stampers (Real Policy Rate + Import Cover, which carried the identical latent bug) now key on the exact reconstructed note.

### Added
- **An honest year-ago value in the editor's chart digest** (#185): "down from 9.95% a year earlier" printed the 24-month *window start* (two years back; the true year-ago point was 9.77) because `first_value` was the only "then" number the editor ever received. `series_summary` now carries `value_1y_ago`/`ts_1y_ago` (nearest point to last−365d, never a silent stand-in beyond 45 days), and a WARN-mode check flags a "year earlier/ago" claim citing the window start — corpus-replayed with zero false positives and the real issue-205 defect pinned as a regression test.
- **A machine-computed session-low fact for the DSE section** (#185): the pipeline computes "sessions since a lower close" from the DSEX series it already fetches and feeds it through the history-facts-verbatim contract (e.g. "a 42-session low (5,722.21 on 23 Aug the last lower close)") — no fact below rank 3, under 30 points, or at a window low. The editor no longer has a blank where a rank should be, which is how "a ten-session low" fossilized across four editions.
- **The Real Policy Rate card shows its arithmetic** (#186): a deterministic post-editor stamp appends both legs and the decision date — "9.50% repo (30 Jul cut) − 8.32% Jul p2p CPI" — read from the builder's own output every run, so a wrong leg is visible on the page, not just in the pipeline.
...
### Added
- **Light/dark theme** (#179): steel-crimson-only dark palette, a masthead + sticky-bar toggle whose choice persists per device (first visits follow the OS), pre-paint FOUC guard, and chart re-theming — every Chart.js canvas rebuilds in the new theme's inks on flip via `lib/useTheme.ts`.
- **Installable PWA** (#180): web app manifest, regenerated band-identity icons (dark "B." mark + maskable variant), `viewport-fit: cover` with full safe-area inset coverage. Online-only by decision — no service worker yet.
- **The ink band** (PR C): a true full-bleed `#0B0F12` masthead band, ink in both themes, with band-scoped type; the sticky bar matches, so the identity anchor keeps its shape on scroll. iOS status bar goes `black-translucent` now that the top of the document is ink in both themes.
- **Snapshot strip on real issues** (PR C): six canonical market cells (USD/BDT, DSEX, 91d T-Bill, Brent, Gold, Remittance) derive from section metrics when no snapshot section exists — which is every real issue today; the strip and its scroll-spy target render for the first time.

### Changed
- **Compact 1c density** (PR C): the ratified review-card scale — wordmark clamp(40–54px), section titles 28px inline with their `§NN · Group` eyebrow, banker runway 44px, verdicts 17px, tile/label floors at 9px `--ink-3`; nothing on the page renders larger than the wordmark at any viewport.
- **KPI tile grid** (PR C): metrics render as grout-gap tiles (label → value → sub → vintage) instead of the vertical rail; summary pills merge into the same grid, deduplicated against metric values.
- **Crimson budget restated** (PR C): five accent jobs — live pulse + chart-latest, Today's Call label, bear tone, active/lead nav, underlines. FIG labels, submit-hover fills and verdict emphasis come off accent.
- **Print contract** (PR C): full light-token forcing at body and html level in both print paths (`?print=1` and native), the band prints as paper, charts rebuild in light inks via the theme force, print scale compacted to match.

### Removed
- **Cover / "Today's Number"** (PR C, SPA-only): every metric still renders in its section and the snapshot strip carries the top-of-page numbers; the pipeline's `cover_metric` field is untouched. The masthead's tag chips, tagline block (first sentence now lives in the footer line) and boxed Subscribe button (now a band link) are gone with the 1c nameplate row.

### Fixed
- **User-visible:** issue 209 (27 Aug 2026) never published. The sub-editor timed out five consecutive times at 600s each and the pipeline held, correctly leaving issue 208 on the front page rather than shipping an unreviewed brief — but subscribers got no brief that morning. The cause was not a transient outage: the per-attempt deadline had been set at roughly the job's own median runtime, so about half of all attempts were failing by construction. The two most recent successes measured 589.2s (issue 207) and 585.9s (issue 208) — 98% and 97.7% of the 600s allowed — and the pattern across three days reflects that margin exactly: zero timeouts on 25 Aug, two on 26 Aug before the third attempt landed, five on 27 Aug. Retries cannot rescue a call that is slow rather than broken, so five attempts at a starved deadline bought nothing. The sub-editor now gets **1200s per attempt across 3 attempts** (61 min of worst-case budget including backoff, against a ~11-min typical editor draft and brief.service's 90-min `TimeoutStartSec`) instead of 600s across 5. Note the editor, whose draft is the longer job at 530–670s, had 1800s throughout — the starvation was specific to the reviewer. Three new tests parse the retry literals and the installed unit file and fail if the configured budget ever exceeds the systemd cap again, which is the comparison nothing in the suite was making.
- The prompt rule against carrying yesterday's prose forward (previous entry) only half-landed, so the fix moves to the input. Measured on issue 208, the first written under that rule, against 207: mean section similarity fell 85.5% → 76.7% and seven of nine sections were genuinely rewritten — `macro` broke out of its 100.0% byte-identical lock to 83.4%, `remit` went 98.6% → 32.8%, `dse` 76.7% → 41.6% — and the verbless clause quoted as the voice block's own BAD example is now gone from every section, replaced by correctly re-derived figures. No over-correction appeared: no manufactured movement, no padding with extra figures. But `banking` **regressed from 50.9% to 97.4%** and `fiscal` held at 96.2%, both satisfying the rule cosmetically — "hold 32.26%" became "hold at 32.26%", "Neither number" became "Neither figure". A rule against reuse is satisfiable by fake edits. The editor therefore no longer receives yesterday's prose at all: `previous_brief` is reduced to a structural skeleton (per section slug/ord/weight/title/group_key/verdict_tone/freshness plus metric labels; brief-level issue_no/volume/lens/frame plus `cover_metric.{section_slug,tone}`) via a new `_previous_brief_skeleton`, built from an allowlist so a prose field added to the schema later is excluded by default. That covers every use of `previous_brief` the editor prompt documents — hero rotation and stance-flip detection — while every other continuity signal already arrives by a path that reads the real unscrubbed object earlier in the same function (`is_held_over` via `_index_previous_metrics`, headline de-duplication via `filter_headlines`, lens rotation via the separate `previous_lens` argument), none of which changes. The **sub-editor still receives the full number-scrubbed prose**, so the reviewer can still catch two issues landing near-identical — now as convergence rather than copying — and its §7 check, which compares wording, does not become a silent no-op. Side effect: the editor's `previous_brief` payload drops from ~63,000 characters to ~2,400 on real production data, freeing input headroom against the editor's known response ceiling. What is deliberately given up: the editor can no longer see that it already made a point yesterday and pick a different angle; restating an unmoved fact in today's words is the correct output and the voice rules say so.
- The v2.2.0 voice change reached only the sections the editor chose to rewrite. Measured on issue 207 (the first brief in the new register) against issue 206: `macro` came back **100.0% byte-identical** to the previous day, with `remit` 98.6%, `tbond` 97.7%, `iran` 96.7% and `fiscal` 94.4% — five of nine sections carried forward near-verbatim, keeping the old telegraphic register including the exact verbless clause the voice block quotes as its BAD example ("cover 5.96mo on the May read"). Only `banking` (50.9%) and `bb` (65.7%) were genuinely rewritten, and those are precisely the two where the new voice landed. Cause: `pipeline_v6._build_editor_input` hands the editor the previous issue as `previous_brief` with every word intact and only the numbers scrubbed (the 2026-08-22 anti-fossilisation fix), so where the data had not moved the editor restated yesterday's sentence. Both editor prompts gain a fifth Register rule — WRITE TODAY'S SENTENCES — forbidding verbatim reuse while explicitly barring the opposite failure (manufacturing movement or reaching for extra figures to look different). The sub-editor, which receives the same `previous_brief`, gains a matching revise-level check; its rewrite is bound by §14, so figures and period strings must survive byte-identical and it is told to log rather than risk the publish if a section cannot be freshened without touching a number. Note the neutrality half of v2.2.0 landed in full — both of its BAD examples are gone from every section they appeared in.
