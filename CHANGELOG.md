...
### Fixed
- **§02 and §05's reserves badge carries information again** (#TBD): `gross_reserves_usd_bn` has been month-END stamped since econdelta #97 (1 Aug 2026), with the new month's figure landing ~11 days after month-end — but both `bb.py` and `fx.py` declared it `cadence="weekly"` (fresh ≤7 days, stale >10). A print on that clock is stale on arrival and stays stale until the next one lands, so Policy & Rates and FX & External read "stale" in every issue since #98 (117/117 checked) whatever the data did. Both builders now declare `monthly` (fresh ≤35 days, warning ≤45, stale beyond), in lockstep — a test pins that the two can never split again — so a genuinely late BB print still drags the badge while a current one no longer does. Nothing about the value or its date changes; only the badge.
