...
### Added
- **The yield ladder names the auction it was built from** (#204): a footnote under the chart — "Curve built from auctions through 27 Aug 2026". The ladder's month rungs are stamped to the FIRST of the month, which is a bucket label, not an observation date; now that the newest line can be a *partial* month, "Aug 2026" alone doesn't tell a reader whether it means the 5th or the 27th. The date is the real per-tenor auction date EconDelta already records in `metric_history_monthly.source_as_of`, taken as the newest across the eight tenors of the newest month. Absent on older issues whose rows predate that column — the footnote then simply doesn't render.
