...
[![Version](https://img.shields.io/badge/version-2.3.1-a67c2e?style=flat-square)](./CHANGELOG.md)
...
**Install it →** open the site on your phone → Share → *Add to Home Screen* (ships as an installable app since v2.3.0)
...
| Light | Dark |
|---|---|
| ![The Brief — light theme](assets/screenshot-light.png) | ![The Brief — dark theme](assets/screenshot-dark.png) |

<p align="center"><img src="assets/screenshot-mobile.png" alt="The Brief on mobile — dark theme" width="320"></p>
...
2. **The Brief pipeline** — Python, runs on Hetzner via `brief.service` systemd unit. Fires at 08:00 BDT. Six stages:
   ...
   4. **Subeditor LLM + deterministic gates** — a second Claude call reviews the editor's output against the Pydantic schema + editorial rules (a malformed or incomplete review is retried once, then HOLDS the publish rather than shipping unreviewed), followed by a hard-coded fact-checker: every currency/percent/bp figure in the prose must trace to a builder value within tolerance, machine-computed disclosures (import-cover periods, the Real Policy Rate's own arithmetic) are stamped onto the page deterministically, and a fabricated count claim — a "ten-session low" no data supplied — warns everywhere and hard-fails the publish in the DSE section, where the pipeline computes the true rank itself
...
├── tests/                        # pytest suite (1,100+ tests as of v2.3.1)
...
See [CHANGELOG.md](./CHANGELOG.md) for the full release history. Current: **v2.3.1** — the honesty patch (fabricated-claim hard block, Real Policy Rate correction, provenance stamps) on top of v2.3.0's 1c facelift.
