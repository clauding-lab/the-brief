"""Phase A — wire fiscal/remit/comm builders into the V6 pipeline.

The 3 builders already exist in brief/builders/ and produce valid
SectionData. Phase A:
...
Slots:
  ...
  - comm   → ord 12, group "markets" (after remit)
...
@pytest.mark.parametrize("builder_id", ["fiscal", "remit", "comm"])
...
def test_to_v6_raw_includes_fiscal_remit_comm_alongside_existing_sections():
    """Build a list with banking + fiscal + remit + comm + iranwar; assert all 5
    ...
    assert slugs == ["banking", "fiscal", "iran", "remit", "comm"], (
        f"Output must include all 5 sections sorted by ord ascending; got {slugs}"
    )


def test_to_v6_raw_carries_fiscal_remit_comm_slug_ord_group():
    """Each Phase A section retains the right slug/ord/group_key after transform."""
    sections = [_section("fiscal"), _section("remit"), _section("comm")]
    out = pipeline_v6._to_v6_raw(sections)
    ...
    assert by_slug["comm"]["ord"] == 12
    assert by_slug["comm"]["group_key"] == "markets"
